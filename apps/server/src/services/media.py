"""
Media utilities for extracting frames from video files.
Uses ffmpeg subprocess for frame extraction.
"""
import asyncio
import subprocess
import os


async def extract_frame(video_path: str, timestamp_s: float) -> bytes:
    """
    Extract a single frame from a video at the given timestamp.
    Returns JPEG bytes.

    Requires ffmpeg installed on the system.
    Falls back to a placeholder frame if ffmpeg is not available or video doesn't exist.
    """
    if not os.path.exists(video_path):
        # Return a minimal valid JPEG placeholder (1x1 pixel)
        return _placeholder_frame()

    try:
        # Run ffmpeg in a thread to avoid blocking the event loop
        result = await asyncio.to_thread(
            _extract_frame_sync, video_path, timestamp_s
        )
        return result
    except Exception:
        return _placeholder_frame()


def _extract_frame_sync(video_path: str, timestamp_s: float) -> bytes:
    """Synchronous frame extraction using ffmpeg."""
    cmd = [
        "ffmpeg",
        "-ss", str(timestamp_s),
        "-i", video_path,
        "-frames:v", "1",
        "-f", "image2",
        "-c:v", "mjpeg",
        "-q:v", "2",
        "pipe:1",
    ]
    result = subprocess.run(
        cmd,
        capture_output=True,
        timeout=10,
    )
    if result.returncode != 0:
        raise RuntimeError(f"ffmpeg failed: {result.stderr.decode()}")
    return result.stdout


def _placeholder_frame() -> bytes:
    """Return a minimal JPEG placeholder (1x1 white pixel)."""
    # Minimal valid JPEG: 1x1 white pixel
    return bytes([
        ,
    ])
